...
// I couldn't figure out where SIZE_MAX was defined for VC6. If anybody knows,
// let me know.
...
static const drwav_uint8 drwavGUID_W64_JUNK[16] = {
...
static DRWAV_INLINE unsigned short drwav__bytes_to_u16(
...
static DRWAV_INLINE unsigned int drwav__bytes_to_u32(
    const unsigned char* data) {
  if (drwav__is_little_endian()) {
...
static DRWAV_INLINE drwav_uint64
drwav__bytes_to_u64(const unsigned char* data) {
  if (drwav__is_little_endian()) {
    ...
    return ((drwav_uint64)data[7] << 0) | ((drwav_uint64)data[6] << 8) |
           ((drwav_uint64)data[5] << 16) | ((drwav_uint64)data[4] << 24) |
           ((drwav_uint64)data[3] << 32) | ((drwav_uint64)data[2] << 40) |
           ((drwav_uint64)data[1] << 48) | ((drwav_uint64)data[0] << 56);
  }
}
...
drwav_uint64 drwav_read_s16__msadpcm(drwav* pWav, drwav_uint64 samplesToRead,
                                     drwav_int16* pBufferOut);
drwav_uint64 drwav_read_s16__ima(drwav* pWav, drwav_uint64 samplesToRead,
                                 drwav_int16* pBufferOut);
...
static drwav_bool32 drwav__read_chunk_header(drwav_read_proc onRead,
                                             void* pUserData,
                                             drwav_container container,
                                             drwav_uint64* pRunningBytesReadOut,
                                             drwav__chunk_header* pHeaderOut) {
...
static drwav_bool32 drwav__read_fmt(drwav_read_proc onRead,
  ...
  if (!drwav__read_chunk_header(onRead, pUserData, container,
                                pRunningBytesReadOut, &header)) {
    return DRWAV_FALSE;
  }

  // Skip junk chunks.
  if ((container == drwav_container_riff &&
       drwav__fourcc_equal(header.id.fourcc, "JUNK")) ||
      (container == drwav_container_w64 &&
       drwav__guid_equal(header.id.guid, drwavGUID_W64_JUNK))) {
    ...
    *pRunningBytesReadOut += header.sizeInBytes + header.paddingSize;
    ...
    return drwav__read_fmt(onRead, onSeek, pUserData, container,
                           pRunningBytesReadOut, fmtOut);
  }
...
drwav_bool32 drwav_init_file(drwav* pWav, const char* filename) {
  ...
  return drwav_init(pWav, drwav__on_read_stdio, drwav__on_seek_stdio,
                    (void*)pFile);
}
...
drwav_bool32 drwav_init_file_write(drwav* pWav, const char* filename,
  ...
  return drwav_init_write(pWav, pFormat, drwav__on_write_stdio,
...
drwav* drwav_open_file(const char* filename) {
  ...
  drwav* pWav =
      drwav_open(drwav__on_read_stdio, drwav__on_seek_stdio, (void*)pFile);
  if (pWav == NULL) {
    fclose(pFile);
    return NULL;
  }
...
drwav* drwav_open_file_write(const char* filename,
  ...
  drwav* pWav = drwav_open_write(pFormat, drwav__on_write_stdio,
                                 drwav__on_seek_stdio, (void*)pFile);
  if (pWav == NULL) {
    fclose(pFile);
    return NULL;
...
static size_t drwav__on_read_memory(void* pUserData, void* pBufferOut,
  ...
  if (bytesToRead > 0) {
    DRWAV_COPY_MEMORY(pBufferOut, memory->data + memory->currentReadPos,
                      bytesToRead);
  ...
  if (origin == drwav_seek_origin_current) {
    if (offset > 0) {
      if (memory->currentReadPos + offset > memory->dataSize) {
        offset =
            (int)(memory->dataSize -
                  memory->currentReadPos);  // Trying to seek too far forward.
      }
    } else {
      if (memory->currentReadPos < (size_t)-offset) {
        offset =
            -(int)memory->currentReadPos;  // Trying to seek too far backwards.
      }
    }
    ...
    if ((drwav_uint32)offset <= memory->dataSize) {
      ...
      memory->currentReadPos =
          memory->dataSize;  // Trying to seek too far forward.
    }
  }
...
static size_t drwav__on_write_memory(void* pUserData, const void* pDataIn,
  ...
  if (bytesRemaining < bytesToWrite) {
    ...
    size_t newDataCapacity =
        (memory->dataCapacity == 0) ? 256 : memory->dataCapacity * 2;
  ...
  if (origin == drwav_seek_origin_current) {
    if (offset > 0) {
      if (memory->currentWritePos + offset > memory->dataSize) {
        offset =
            (int)(memory->dataSize -
                  memory->currentWritePos);  // Trying to seek too far forward.
      }
    } else {
      if (memory->currentWritePos < (size_t)-offset) {
        offset =
            -(int)memory->currentWritePos;  // Trying to seek too far backwards.
      }
    }
...
drwav_bool32 drwav_init_memory(drwav* pWav, const void* data, size_t dataSize) {
  ...
  if (!drwav_init(pWav, drwav__on_read_memory, drwav__on_seek_memory,
                  (void*)&memoryStream)) {
    return DRWAV_FALSE;
  }
...
drwav_bool32 drwav_init_memory_write(drwav* pWav, void** ppData,
  ...
  *pDataSize = 0;
  ...
  if (!drwav_init_write(pWav, pFormat, drwav__on_write_memory,
...
drwav* drwav_open_memory(const void* data, size_t dataSize) {
  ...
  drwav* pWav = drwav_open(drwav__on_read_memory, drwav__on_seek_memory,
                           (void*)&memoryStream);
  if (pWav == NULL) {
    return NULL;
  }
...
drwav* drwav_open_memory_write(void** ppData, size_t* pDataSize,
  ...
  *pDataSize = 0;
  ...
  drwav* pWav =
      drwav_open_write(pFormat, drwav__on_write_memory,
...
drwav_bool32 drwav_init(drwav* pWav, drwav_read_proc onRead,
  ...
  if (pWav->container == drwav_container_riff) {
    ...
    if (onRead(pUserData, chunkSizeBytes, sizeof(chunkSizeBytes)) !=
  ...
  drwav_fmt fmt;
  if (!drwav__read_fmt(onRead, onSeek, pUserData, pWav->container,
                       &pWav->dataChunkDataPos, &fmt)) {
  ...
  for (;;) {
    drwav__chunk_header header;
    if (!drwav__read_chunk_header(onRead, pUserData, pWav->container,
                                  &pWav->dataChunkDataPos, &header)) {
      return DRWAV_FALSE;
    }
    ...
    if (pWav->container == drwav_container_riff) {
      if (drwav__fourcc_equal(header.id.fourcc, "fact")) {
        ...
        // understanding it properly. For now I am only enabling this for
        // Microsoft ADPCM formats.
  ...
  // The bytes per sample should never be 0 at this point. This would indicate
  // an invalid WAV file.
  ...
  if (sampleCountFromFactChunk != 0) {
    ...
    if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM) {
      ...
      pWav->totalSampleCount =
          (blockCount * (fmt.blockAlign - (6 * pWav->channels))) *
          2;  // x2 because two samples per byte.
    }
  ...
  // The way we calculate the bytes per sample does not make sense for
  // compressed formats. We set this to 0 for compressed WAVs.
  ...
  // using (from the Windows installer on the libsndfile website), it appears
  // the total sample count libsndfile uses for MS-ADPCM is incorrect. It would
  // seem they are computing the total sample count from the number of blocks,
  // however this results in the inclusion of the extra silent samples at the
  // end of the last block. The correct way to know the total sample count is to
  // inspect the "fact" chunk which should always be present for compressed
  // formats, and should always include the sample count. This little block of
  // code below is only used to emulate the libsndfile logic so I can properly
  // run my correctness tests against libsndfile, and is disabled by default.
  if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM) {
    drwav_uint64 blockCount = dataSize / fmt.blockAlign;
    pWav->totalSampleCount =
        (blockCount * (fmt.blockAlign - (6 * pWav->channels))) *
...
drwav_bool32 drwav_init_write(drwav* pWav, const drwav_data_format* pFormat,
  ...
  pWav->fmt.avgBytesPerSec = (drwav_uint32)(
      (pFormat->bitsPerSample * pFormat->sampleRate * pFormat->channels) >> 3);
  pWav->fmt.blockAlign =
      (drwav_uint16)((pFormat->channels * pFormat->bitsPerSample) >> 3);
...
void drwav_uninit(drwav* pWav) {
  ...
  // If the drwav object was opened in write mode we'll need to finialize a few
  // things:
      ...
      if (pWav->container == drwav_container_riff) {
        ...
        if (pWav->onSeek(pWav->pUserData, (int)pWav->dataChunkDataPos + 16,
          ...
          drwav_uint64 dataChunkSize =
              24 + pWav->dataChunkDataSize;  // +24 because W64 includes the
                                             // size of the GUID and size
                                             // fields.
...
drwav_uint64 drwav_read(drwav* pWav, drwav_uint64 samplesToRead,
  ...
  size_t bytesRead = drwav_read_raw(
      pWav, (size_t)(samplesToRead * pWav->bytesPerSample), pBufferOut);
...
drwav_bool32 drwav_seek_to_sample(drwav* pWav, drwav_uint64 sample) {
  ...
  // forward we just seek forward. If we are going backwards we need to seek
  // back to the start.
  if (drwav__is_compressed_format_tag(pWav->translatedFormatTag)) {
    // TODO: This can be optimized.

    // If we're seeking forward it's simple - just keep reading samples until we
    // hit the sample we're requesting. If we're seeking backwards, we first
    ...
    if (sample > pWav->compressed.iCurrentSample) {
      ...
      while (offset > 0) {
        ...
        if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM) {
          ...
          assert(DRWAV_FALSE);  // If this assertion is triggered it means I've
    ...
    drwav_uint64 totalSizeInBytes =
        pWav->totalSampleCount * pWav->bytesPerSample;
  ...
  while (samplesToRead > 0 &&
    ...
    if (pWav->msadpcm.cachedSampleCount == 0 &&
      ...
      if (pWav->channels == 1) {
        ...
        pWav->msadpcm.prevSamples[0][1] =
            (drwav_int32)drwav__bytes_to_s16(header + 3);
        pWav->msadpcm.prevSamples[0][0] =
    ...
    while (samplesToRead > 0 && pWav->msadpcm.cachedSampleCount > 0 &&
      ...
      pBufferOut[0] =
          (drwav_int16)pWav->msadpcm
              .cachedSamples[drwav_countof(pWav->msadpcm.cachedSamples) -
    ...
    if (pWav->msadpcm.cachedSampleCount == 0) {
      if (pWav->msadpcm.bytesRemainingInBlock == 0) {
        ...
        if (pWav->channels == 1) {
          ...
          newSample0 = ((pWav->msadpcm.prevSamples[0][1] *
          ...
          newSample1 = ((pWav->msadpcm.prevSamples[0][1] *
                         coeff1Table[pWav->msadpcm.predictor[0]]) +
                        (pWav->msadpcm.prevSamples[0][0] *
                         coeff2Table[pWav->msadpcm.predictor[0]])) >>
                       8;
          ...
          // Right.
          drwav_int32 newSample1;
          newSample1 = ((pWav->msadpcm.prevSamples[1][1] *
  ...
  while (samplesToRead > 0 &&
    ...
    if (pWav->ima.cachedSampleCount == 0 &&
      ...
      if (pWav->channels == 1) {
        ...
        pWav->ima.cachedSamples[drwav_countof(pWav->ima.cachedSamples) - 1] =
        ...
        pWav->ima.cachedSamples[drwav_countof(pWav->ima.cachedSamples) - 2] =
            pWav->ima.predictor[0];
        pWav->ima.cachedSamples[drwav_countof(pWav->ima.cachedSamples) - 1] =
    ...
    while (samplesToRead > 0 && pWav->ima.cachedSampleCount > 0 &&
      ...
      pBufferOut[0] =
          (drwav_int16)
              pWav->ima.cachedSamples[drwav_countof(pWav->ima.cachedSamples) -
    ...
    if (pWav->ima.cachedSampleCount == 0) {
      if (pWav->ima.bytesRemainingInBlock == 0) {
        ...
        // From what I can tell with stereo streams, it looks like every 4 bytes
        // (8 samples) is for one channel. So it goes 4 bytes for the left
        // channel, 4 bytes for the right channel.
        ...
        for (drwav_uint32 iChannel = 0; iChannel < pWav->channels; ++iChannel) {
          ...
          for (drwav_uint32 iByte = 0; iByte < 4; ++iByte) {
            ...
            pWav->ima
                .cachedSamples[(drwav_countof(pWav->ima.cachedSamples) -
                                pWav->ima.cachedSampleCount) +
...
static void drwav__pcm_to_s16(drwav_int16* pOut, const unsigned char* pIn,
...
static void drwav__ieee_to_s16(drwav_int16* pOut, const unsigned char* pIn,
                               size_t totalSampleCount,
                               unsigned short bytesPerSample) {
  if (bytesPerSample == 4) {
...
drwav_uint64 drwav_read_s16__pcm(drwav* pWav, drwav_uint64 samplesToRead,
                                 drwav_int16* pBufferOut) {
  // Fast path.
  ...
  while (samplesToRead > 0) {
    drwav_uint64 samplesRead = drwav_read(
        pWav,
        drwav_min(samplesToRead, sizeof(sampleData) / pWav->bytesPerSample),
        sampleData);
    if (samplesRead == 0) {
      break;
    }
...
drwav_uint64 drwav_read_s16__ieee(drwav* pWav, drwav_uint64 samplesToRead,
                                  drwav_int16* pBufferOut) {
  drwav_uint64 totalSamplesRead = 0;
  unsigned char sampleData[4096];
  while (samplesToRead > 0) {
...
drwav_uint64 drwav_read_s16__alaw(drwav* pWav, drwav_uint64 samplesToRead,
                                  drwav_int16* pBufferOut) {
  drwav_uint64 totalSamplesRead = 0;
  unsigned char sampleData[4096];
  while (samplesToRead > 0) {
...
drwav_uint64 drwav_read_s16__mulaw(drwav* pWav, drwav_uint64 samplesToRead,
                                   drwav_int16* pBufferOut) {
  drwav_uint64 totalSamplesRead = 0;
  unsigned char sampleData[4096];
  while (samplesToRead > 0) {
...
void drwav_s24_to_s16(drwav_int16* pOut, const drwav_uint8* pIn,
  ...
  int r;
  for (size_t i = 0; i < sampleCount; ++i) {
    int x = ((int)(((unsigned int)(((unsigned char*)pIn)[i * 3 + 0]) << 8) |
                   ((unsigned int)(((unsigned char*)pIn)[i * 3 + 1]) << 16) |
                   ((unsigned int)(((unsigned char*)pIn)[i * 3 + 2])) << 24)) >>
            8;
    r = x >> 8;
    pOut[i] = (short)r;
  }
...
void drwav_f64_to_s16(drwav_int16* pOut, const double* pIn,
                      size_t sampleCount) {
  int r;
  for (size_t i = 0; i < sampleCount; ++i) {
    double x = pIn[i];
...
drwav_uint64 drwav_read_f32__pcm(drwav* pWav, drwav_uint64 samplesToRead,
                                 float* pBufferOut) {
  if (pWav->bytesPerSample == 0) {
    return 0;
  }
...
drwav_uint64 drwav_read_f32__msadpcm(drwav* pWav, drwav_uint64 samplesToRead,
                                     float* pBufferOut) {
  ...
  while (samplesToRead > 0) {
    drwav_uint64 samplesRead =
        drwav_read_s16(pWav, drwav_min(samplesToRead, 2048), samples16);
    if (samplesRead == 0) {
      break;
    }
    ...
    drwav_s16_to_f32(
        ...
        (size_t)samplesRead);  // <-- Safe cast because we're clamping to 2048.
...
drwav_uint64 drwav_read_f32__ima(drwav* pWav, drwav_uint64 samplesToRead,
                                 float* pBufferOut) {
  // We're just going to borrow the implementation from the drwav_read_s16()
...
drwav_uint64 drwav_read_f32__ieee(drwav* pWav, drwav_uint64 samplesToRead,
                                  float* pBufferOut) {
  // Fast path.
  if (pWav->translatedFormatTag == DR_WAVE_FORMAT_IEEE_FLOAT &&
      pWav->bytesPerSample == 4) {
    return drwav_read(pWav, samplesToRead, pBufferOut);
  }
...
drwav_uint64 drwav_read_f32__alaw(drwav* pWav, drwav_uint64 samplesToRead,
                                  float* pBufferOut) {
  if (pWav->bytesPerSample == 0) {
    return 0;
  }
...
drwav_uint64 drwav_read_f32__mulaw(drwav* pWav, drwav_uint64 samplesToRead,
                                   float* pBufferOut) {
  if (pWav->bytesPerSample == 0) {
    return 0;
  }
...
void drwav_u8_to_f32(float* pOut, const drwav_uint8* pIn, size_t sampleCount) {
  ...
  // therefore leaving this block here just for my automated correctness
...
void drwav_alaw_to_f32(float* pOut, const drwav_uint8* pIn,
                       size_t sampleCount) {
  if (pOut == NULL || pIn == NULL) {
    return;
  }
...
static void drwav__pcm_to_s32(drwav_int32* pOut, const unsigned char* pIn,
                              size_t totalSampleCount,
                              unsigned short bytesPerSample) {
  // Special case for 8-bit sample data because it's treated as unsigned.
  if (bytesPerSample == 1) {
...
static void drwav__ieee_to_s32(drwav_int32* pOut, const unsigned char* pIn,
                               size_t totalSampleCount,
                               unsigned short bytesPerSample) {
  if (bytesPerSample == 4) {
...
drwav_uint64 drwav_read_s32__pcm(drwav* pWav, drwav_uint64 samplesToRead,
                                 drwav_int32* pBufferOut) {
  // Fast path.
  if (pWav->translatedFormatTag == DR_WAVE_FORMAT_PCM &&
      pWav->bytesPerSample == 4) {
...
drwav_uint64 drwav_read_s32__msadpcm(drwav* pWav, drwav_uint64 samplesToRead,
                                     drwav_int32* pBufferOut) {
  // We're just going to borrow the implementation from the drwav_read_s16()
  // since ADPCM is a little bit more complicated than other formats and I don't
  // want to duplicate that code.
...
drwav_uint64 drwav_read_s32__ima(drwav* pWav, drwav_uint64 samplesToRead,
                                 drwav_int32* pBufferOut) {
  // We're just going to borrow the implementation from the drwav_read_s16()
  // since IMA-ADPCM is a little bit more complicated than other formats and I
  // don't want to duplicate that code.
...
drwav_uint64 drwav_read_s32__ieee(drwav* pWav, drwav_uint64 samplesToRead,
                                  drwav_int32* pBufferOut) {
  if (pWav->bytesPerSample == 0) {
    return 0;
  }
...
drwav_uint64 drwav_read_s32__alaw(drwav* pWav, drwav_uint64 samplesToRead,
                                  drwav_int32* pBufferOut) {
  if (pWav->bytesPerSample == 0) {
    return 0;
  }
...
drwav_uint64 drwav_read_s32__mulaw(drwav* pWav, drwav_uint64 samplesToRead,
                                   drwav_int32* pBufferOut) {
  if (pWav->bytesPerSample == 0) {
    return 0;
  }
...
void drwav_f64_to_s32(drwav_int32* pOut, const double* pIn,
                      size_t sampleCount) {
  if (pOut == NULL || pIn == NULL) {
    return;
  }
...
drwav_int16* drwav__read_and_close_s16(drwav* pWav, unsigned int* channels,
                                       unsigned int* sampleRate,
                                       drwav_uint64* totalSampleCount) {
  drwav_assert(pWav != NULL);
  ...
  drwav_int16* pSampleData = (drwav_int16*)DRWAV_MALLOC(
      (size_t)sampleDataSize);  // <-- Safe cast due to the check above.
  if (pSampleData == NULL) {
    drwav_uninit(pWav);
  ...
  drwav_uint64 samplesRead =
      drwav_read_s16(pWav, (size_t)pWav->totalSampleCount, pSampleData);
...
float* drwav__read_and_close_f32(drwav* pWav, unsigned int* channels,
                                 unsigned int* sampleRate,
                                 drwav_uint64* totalSampleCount) {
  drwav_assert(pWav != NULL);
  ...
  float* pSampleData = (float*)DRWAV_MALLOC(
      (size_t)sampleDataSize);  // <-- Safe cast due to the check above.
  if (pSampleData == NULL) {
    drwav_uninit(pWav);
    return NULL;  // Failed to allocate memory.
  ...
  drwav_uint64 samplesRead =
      drwav_read_f32(pWav, (size_t)pWav->totalSampleCount, pSampleData);
  if (samplesRead != pWav->totalSampleCount) {
    DRWAV_FREE(pSampleData);
    drwav_uninit(pWav);
    return NULL;  // There was an error reading the samples.
...
drwav_int32* drwav__read_and_close_s32(drwav* pWav, unsigned int* channels,
                                       unsigned int* sampleRate,
                                       drwav_uint64* totalSampleCount) {
  drwav_assert(pWav != NULL);
  ...
  drwav_int32* pSampleData = (drwav_int32*)DRWAV_MALLOC(
      (size_t)sampleDataSize);  // <-- Safe cast due to the check above.
  if (pSampleData == NULL) {
    drwav_uninit(pWav);
    return NULL;  // Failed to allocate memory.
  ...
  drwav_uint64 samplesRead =
      drwav_read_s32(pWav, (size_t)pWav->totalSampleCount, pSampleData);
  if (samplesRead != pWav->totalSampleCount) {
    DRWAV_FREE(pSampleData);
    drwav_uninit(pWav);
    return NULL;  // There was an error reading the samples.
...
drwav_int16* drwav_open_and_read_s16(drwav_read_proc onRead,
  ...
  return drwav__read_and_close_s16(&wav, channels, sampleRate,
                                   totalSampleCount);
}
...
float* drwav_open_and_read_f32(drwav_read_proc onRead, drwav_seek_proc onSeek,
  ...
  return drwav__read_and_close_f32(&wav, channels, sampleRate,
                                   totalSampleCount);
}
...
drwav_int32* drwav_open_and_read_s32(drwav_read_proc onRead,
  ...
  return drwav__read_and_close_s32(&wav, channels, sampleRate,
                                   totalSampleCount);
}
...
//   dr_wav will output silence for
...
//
// v0.5e - 2016-12-29
//   - Added support for reading samples as signed 16-bit integers. Use the
//   _s16() family of APIs for this.
...
//
// v0.5a - 2016-10-11
//   - Fixed a bug with drwav_open_and_read() and family due to incorrect
//   argument ordering.
//   - Improve A-law and mu-law efficiency.
//
// v0.5 - 2016-09-29
//   - API CHANGE. Swap the order of "channels" and "sampleRate" parameters in
//   drwav_open_and_read*(). Rationale for this is to
...
//
// v0.1a - 2016-05-07
//   - Fixed a bug in drwav_open_file() where the file handle would not be
//   closed if the loader failed to initialize.
//
// v0.1 - 2016-05-04
